...
class CATSeg(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if 'eva' in self.clip_name.lower():
           clip_features= self.sem_seg_head.predictor.clip_model.encode_dense(clip_images_resized,
                                                                                normalize=True,
                                                                                keep_shape=False,
                                                                                mode='csa',) # bs, N, C
        ...
        if 'eva' in self.clip_name.lower():
            res4 = rearrange(self.layers[0][:, 1:, :], "B (H W) C -> B C H W", H=24)
            res5 = rearrange(self.layers[1][:, 1:, :], "B (H W) C -> B C H W", H=24)
        else:
        ...
        if self.training:
            outputs,loss_organ, similarity = self.sem_seg_head(clip_features, features,need_loss=True)
            ...
            outputs = self.sem_seg_head(clip_features, features,batched_inputs)
    ...
    def inference_sliding_window(self, batched_inputs, kernel=384, overlap=0.333, out_res=[640, 640]):
        ...
        self.layers = []
        if 'eva' in self.clip_name.lower():
           clip_features= self.sem_seg_head.predictor.clip_model.encode_dense(clip_images,
                                                                                normalize=True,
                                                                                keep_shape=False,
                                                                                mode='csa',) # bs, N, C
           image_features=clip_features
        ...
        res5 = self.upsample2(res5)
        features = {'res5': res5, 'res4': res4, 'res3': res3,}
        outputs = self.sem_seg_head(clip_features, features,batched_inputs)  # torch.Size([5, 59, 96, 96])
